...
orders.forEach(function (item) {
    ...
    if (orderDate.getFullYear() == '1996') {
        ordersAt1996.push(item)
    }
});

console.log(ordersAt1996)
...
orders.forEach(function (order) {
    ...
    order.details.forEach(function (item) {
        orderPrice += item.unitPrice*item.quantity*(1-item.discount)
    })
    if (orderPrice > highestOrderPrice) {
        highestOrder = order
        highestOrderPrice = orderPrice
    }
})
...
orders.forEach(function (order) {
    if (order.shipAddress.country.toUpperCase() == 'USA') {
        order.details.forEach(function (item) {
            totalUsa += item.unitPrice*item.quantity*(1-item.discount)
        });
    }
})
